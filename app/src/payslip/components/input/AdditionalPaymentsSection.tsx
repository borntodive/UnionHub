...
export const AdditionalPaymentsSection: React.FC<AdditionalPaymentsSectionProps> = ({
  items,
  onAdd,
  onUpdate,
  onRemove,
  ...
  const handleSLRChange = (index: number, isSLR: boolean) => {
    const item = items[index];
    // If SLR is enabled, force tax to 100 (Basic) and disable selector
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Additional Payments</Text>
        ...
        <View key={index} style={styles.itemCard}>
          ...
          <View style={styles.taxContainer}>
            <Text style={[styles.label, item.isSLR && styles.labelDisabled]}>
              Taxation {item.isSLR && '(forzato a Basic per SLR)'}
            </Text>
            <View style={styles.taxOptions}>
              {TAX_OPTIONS.map((option) => (
                <TouchableOpacity
                  ...
                  onPress={() => !item.isSLR && handleTaxChange(index, option.value)}
                  disabled={item.isSLR}
                >
                  <Text
                    style={[
          ...
          <View style={styles.switchRow}>
            <Text style={styles.switchLabel}>Is a Statuary Leave Refund?</Text>
            <Switch
              value={item.isSLR}
