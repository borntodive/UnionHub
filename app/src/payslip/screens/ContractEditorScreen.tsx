...
export default function ContractEditorScreen() {
  ...
  const [year, setYear] = useState(
    existingContract?.effectiveYear?.toString() ||
  ...
  const [allowance, setAllowance] = useState(
    existingContract?.allowance
      ? (existingContract.allowance * 12).toString()
      : "",
  );
  ...
  const [hasTraining, setHasTraining] = useState(
    !!existingContract?.trainingConfig,
  );
  ...
  const [hasNonBtc, setHasNonBtc] = useState(
    !!existingContract?.trainingConfig?.nonBtc,
  );
  ...
  const [hasLtcBonus, setHasLtcBonus] = useState(
    !!existingContract?.trainingConfig?.bonus,
  );
  ...
  useEffect(() => {
    if (existingContract) {
      ...
      setNonBtcAllowance(
        tc?.nonBtc?.allowance ? (tc.nonBtc.allowance * 12).toString() : "",
      );
      setNonBtcSimRate(
        tc?.nonBtc?.simDiaria?.[0]?.pay?.sectorPay?.toString() || "",
      );
      setHasBtc(!!tc?.btc);
      setBtcAllowance(
        tc?.btc?.allowance ? (tc.btc.allowance * 12).toString() : "",
      );
        ...
        trainingConfig: hasTraining
          ? {
              allowance: trainingAllowance
                ? parseFloat(trainingAllowance) / 12
                : undefined,
              nonBtc: hasNonBtc
                ? {
                    allowance: nonBtcAllowance
                      ? parseFloat(nonBtcAllowance) / 12
                      : 0,
                      ...
                      ? [
                          {
                            ...
                            pay: {
                              ffp: 0,
                              sectorPay: parseFloat(nonBtcSimRate) || 0,
                            },
                          },
                        ]
                      : [],
                        ...
                        ? [
                            {
                              ...
                              pay: {
                                ffp: 0,
                                sectorPay: parseFloat(btcSimRate1) || 0,
                              },
                            },
                          ]
                        : []),
                        ...
                        ? [
                            {
                              ...
                              pay: {
                                ffp: 0,
                                sectorPay: parseFloat(btcSimRate2) || 0,
                              },
                            },
                          ]
                        : []),
