...
import { b as re, c as te, g as ie } from "./references-o1kGQSUn.js";
import { A as oe } from "./arrow-left-BIawl3nD.js";
import { U as le } from "./upload-B89mvuMj.js";
import { L as ne } from "./loader-BqyEZcbY.js";
import { C as ce } from "./circle-check-big-Cy9f-Vvd.js";
...
function fe({ mode: t }) {
  const { id: l } = X(),
    ...
    g = J(),
    b = Y((s) => s.user),
    U = (b == null ? void 0 : b.role) === w.SUPERADMIN,
    { data: r, isLoading: k } = A({
      queryKey: ["user", l],
      queryFn: () => N.getUserById(l),
      enabled: t === "edit" && !!l,
    }),
    { data: L } = A({ queryKey: ["bases"], queryFn: re.getBases }),
    { data: z } = A({ queryKey: ["contracts"], queryFn: te.getContracts }),
    { data: h } = A({ queryKey: ["grades"], queryFn: ie.getGrades }),
    v = y.useRef(null),
    [I, S] = y.useState(null),
    [E, C] = y.useState("idle"),
    [P, R] = y.useState(null),
    {
      ...
      watch: q,
      reset: M,
      formState: { errors: d, isSubmitting: T },
      setError: K,
      setValue: m,
    } = Z({
      defaultValues: {
  ...
  y.useEffect(() => {
    ...
    t === "edit" &&
      ...
      M({
        ...
        rsa: r.rsa ?? !1,
        rls: r.rls ?? !1,
        dataIscrizione: F(r.dataIscrizione),
        dateOfEntry: F(r.dateOfEntry),
        dateOfCaptaincy: F(r.dateOfCaptaincy),
      });
  }, [r, t, M]);
  const j = q("ruolo"),
    B = q("gradeId"),
      ...
      j && z
        ? z.filter(
            (s) =>
              s.nome
                .toLowerCase()
                .includes(j === O.PILOT ? "pilot" : "cabin") || !0,
          )
        : (z ?? []),
    V = j && h ? h.filter((s) => s.ruolo === j) : (h ?? []),
    G = h == null ? void 0 : h.find((s) => s.id === B),
    D = G ? de.includes(G.codice) : !1,
    _ = async (s) => {
      var p;
      ...
      try {
        if (t === "create") {
          const p = {
              ...
              dateOfCaptaincy: D ? f(s.dateOfCaptaincy) : void 0,
            },
